  ...
  const extraction = await callStructured(flint, 'ingest', {
    ...
    input: {
      input: [
        ...
        ``,
        `When the module gates an area behind a check — "DC 13 group Stealth check,`,
        `Success: avoid the encounter, Failure: they attack" — put it in "check".`,
        `Those are printed constantly and are the difference between an obstacle the`,
        `party can slip past and a fight they are forced to have.`,
        ``,
        ...
        ``,
        `When an encounter prints a table of counts by party size — "3 PCs / 4 PCs /`,
        `5 PCs / 6 PCs" — fill in "countByPartySize" keyed by that number, and put the`,
        `4-PC figure in "count". Read as one flat number, such a table gives the wrong`,
        `column and the wrong fight.`,
        ``,
